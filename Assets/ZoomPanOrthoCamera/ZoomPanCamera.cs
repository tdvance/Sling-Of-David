...
public class ZoomPanCamera : MonoBehaviour {
    ...
    [Tooltip("Zoom step (multiplicative)")]
    public float zoomAmount = Mathf.Sqrt(2);
    ...
    void Update() {
        ...
        //zoom
        if (CrossPlatformInputManager.GetAxis(zoomAxis) > 0 && currentZoom * zoomAmount <= maxZoom) {
            currentZoom *= zoomAmount;
            cam.orthographicSize /= zoomAmount;
            Debug.Log("Zoom=" + currentZoom);
        } else if (CrossPlatformInputManager.GetAxis(zoomAxis) < 0 && currentZoom / zoomAmount >= minZoom) {
            currentZoom /= zoomAmount;
            cam.orthographicSize *= zoomAmount;
            Debug.Log("Zoom=" + currentZoom);
